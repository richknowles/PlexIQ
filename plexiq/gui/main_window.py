...
from plexiq.gui.components.hotdog_slider import HotdogSlider
...
class PlexIQMainWindow(QMainWindow):
    ...
    def _create_analysis_tab(self) -> QWidget:
        ...
        # Hotdog slider for deletion threshold (Rule #5: Aesthetic & Delight)
        self.deletion_threshold_slider = HotdogSlider(
            min_val=0.0,
            max_val=1.0,
            default_val=self.config.get('thresholds.min_deletion_score', 0.7),
            label="🌭 Deletion Threshold"
        )
        self.deletion_threshold_slider.value_changed.connect(self._on_threshold_changed)
        layout.addWidget(self.deletion_threshold_slider)
    ...
    def _on_threshold_changed(self, value: float):
        """Handle deletion threshold slider change."""
        self.config.set('thresholds.min_deletion_score', value)
        self.status_bar.showMessage(f"Deletion threshold set to {value:.0%}", 3000)

        # Re-analyze if we have items
        if self.analyzed_items:
            self._reapply_threshold(value)

    def _reapply_threshold(self, threshold: float):
        """Re-evaluate deletion recommendations with new threshold."""
        for item in self.analyzed_items:
            score = item.get('deletion_score', 0)
            item['deletion_recommended'] = score >= threshold

        # Update table and summary
        self.results_table.setAnalysisData(self.analyzed_items)
        recommended = sum(1 for item in self.analyzed_items if item.get('deletion_recommended', False))
        self.summary_label.setText(
            f"Analyzed {len(self.analyzed_items)} items | {recommended} recommended for deletion"
        )
        self.status_bar.showMessage(f"Threshold updated: {recommended} items now recommended for deletion", 3000)
