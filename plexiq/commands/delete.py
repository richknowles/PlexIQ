...
Safety: Defaults to dry-run; requires explicit --execute for actual deletion.
       Triple authorization: type YES, then YES again, then enter password.
"""
    ...
    try:
        ...
        if not dry_run and confirm:
            ...
            if not confirmed:
                ...
                logger.warning("Deletion cancelled at first confirmation prompt")
                return

            if not _verify_triple_authorization(config, logger):
                console.print("[red]Deletion cancelled - authorization failed[/red]")
                return
...
def _verify_triple_authorization(config, logger) -> bool:
    """
    Verify triple authorization: YES, then YES again, then password.
    ...
    """
    stored_hash = config.deletion_password_hash

    if not stored_hash:
        logger.error("Deletion password not configured. Set DELETION_PASSWORD_HASH in .env")
        console.print("[red]Error: Deletion password not configured.[/red]")
        console.print("[yellow]Set DELETION_PASSWORD_HASH in .env file (SHA-256 hash).[/yellow]")
        return False

    console.print(Panel(
        "[yellow]TRIPLE AUTHORIZATION REQUIRED[/yellow]\n"
        "Step 1: Type YES\n"
        "Step 2: Type YES again\n"
        "Step 3: Enter password",
        title="Authorization",
        border_style="yellow"
    ))

    logger.info("Starting triple authorization verification")

    first_yes = Prompt.ask(
        "[yellow]Step 1: Type YES to confirm deletion[/yellow]",
        default=""
    )
    if first_yes.strip().upper() != "YES":
        logger.warning("Triple authorization failed: first YES not entered")
        return False

    second_yes = Prompt.ask(
        "[yellow]Step 2: Type YES again to confirm[/yellow]",
        default=""
    )
    if second_yes.strip().upper() != "YES":
        logger.warning("Triple authorization failed: second YES not entered")
        return False

    password = Prompt.ask(
        "[yellow]Step 3: Enter your deletion password[/yellow]",
        password=True,
        default=""
    )
    ...
    if password_hash == stored_hash:
        logger.info("Triple authorization successful")
        return True
    else:
        logger.warning("Triple authorization failed: incorrect password")
        return False
