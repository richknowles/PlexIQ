...
class Config:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            # Security Settings
            'security': {
                'deletion_password_hash': os.getenv('DELETION_PASSWORD_HASH', ''),
            },
    ...
    def deletion_password_hash(self) -> str:
        ...
        return self.get('security.deletion_password_hash', '')

    def has_deletion_password(self) -> bool:
        """
        Check if deletion password is set.

        Returns:
            True if password hash exists, False otherwise
        """
        password_hash = self.get('security.deletion_password_hash', '')
        return bool(password_hash and len(password_hash) > 0)
